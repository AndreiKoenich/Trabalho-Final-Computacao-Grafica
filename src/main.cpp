...
#define NUM_ALVOS_NAOLINEARES 1
...
void desenha_alvos(Alvo vetor_alvos[])
{
    ...
    for (int i = 0; i < QUANTIDADE_ALVOS; i++)
    {
        if (vetor_alvos[i].dano < MAXIMO_DANO)
        {
            ...
            float aux;
            if(vetor_alvos[i].bbox_minimo.x > vetor_alvos[i].bbox_maximo.x){
                aux = vetor_alvos[i].bbox_maximo.x;
                vetor_alvos[i].bbox_maximo.x = vetor_alvos[i].bbox_minimo.x;
                vetor_alvos[i].bbox_minimo.x = aux;
            }
            if(vetor_alvos[i].bbox_minimo.y > vetor_alvos[i].bbox_maximo.y){
                aux = vetor_alvos[i].bbox_maximo.y;
                vetor_alvos[i].bbox_maximo.y = vetor_alvos[i].bbox_minimo.y;
                vetor_alvos[i].bbox_minimo.y = aux;
            }
            if(vetor_alvos[i].bbox_minimo.z > vetor_alvos[i].bbox_maximo.z){
                aux = vetor_alvos[i].bbox_maximo.z;
                vetor_alvos[i].bbox_maximo.z = vetor_alvos[i].bbox_minimo.z;
                vetor_alvos[i].bbox_minimo.z = aux;
            }
...
void desenha_caixas(){
    ...
    model = Matrix_Scale(0.15f, 0.15f, 0.15f);
            PushMatrix(model);
                model = model * Matrix_Translate(-28.0f, 0.0f, 10.0f);
                PushMatrix(model);
            ...
            PopMatrix(model);
            PushMatrix(model);
                model = model * Matrix_Scale(1.0f, 2.0f, 1.0f) * Matrix_Translate(14.0f, 0.0f, 10.0f);
                glUniformMatrix4fv(g_model_uniform, 1 , GL_FALSE , glm::value_ptr(model));
                glUniform1i(g_object_id_uniform, CAIXA);
                DrawVirtualObject("Crate_Plane.005");
            PopMatrix(model);
        ...
        PushMatrix(model);
            model = model * Matrix_Translate(-10.0f, 0.0f, -2.0f) * Matrix_Rotate_Y(-0.2);
            glUniformMatrix4fv(g_model_uniform, 1 , GL_FALSE , glm::value_ptr(model));
            glUniform1i(g_object_id_uniform, CAIXA);
            DrawVirtualObject("Crate_Plane.005");
        PopMatrix(model);
        ...
        PushMatrix(model);
            model = model * Matrix_Translate(15.0f, 0.0f, -5.0f) * Matrix_Rotate_Y(-0.4) * Matrix_Scale(3.0f, 0.5f, 1.0f);
            glUniformMatrix4fv(g_model_uniform, 1 , GL_FALSE , glm::value_ptr(model));
            glUniform1i(g_object_id_uniform, CAIXA);
            DrawVirtualObject("Crate_Plane.005");
        PopMatrix(model);
        ...
        PushMatrix(model);
            model = model * Matrix_Translate(-50.0f, 0.0f, 45.0f) * Matrix_Scale(0.8f, 0.8f, 0.8f) * Matrix_Rotate_Y(0.1);
            glUniformMatrix4fv(g_model_uniform, 1 , GL_FALSE , glm::value_ptr(model));
            glUniform1i(g_object_id_uniform, CAIXA);
            DrawVirtualObject("Crate_Plane.005");
        PopMatrix(model);
        ...
        PushMatrix(model);
            model = model * Matrix_Translate(-50.0f, 0.0f, 35.0f) * Matrix_Scale(0.8f, 0.8f, 0.8f) * Matrix_Rotate_Y(-0.21);
            glUniformMatrix4fv(g_model_uniform, 1 , GL_FALSE , glm::value_ptr(model));
            glUniform1i(g_object_id_uniform, CAIXA);
            DrawVirtualObject("Crate_Plane.005");
        PopMatrix(model);
...
void movimenta_alvos(Alvo vetor_alvos[], double t, glm::vec4 vetor_bezier_alvo)
{
...
int main(int argc, char* argv[])
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 600, "INF01047 - 00308680 - Andrei Pochmann Koenich", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (iniciar_jogo && !fim_jogo)
        {
            ...
            movimenta_alvos(vetor_alvos, tempo_alvo, vetor_bezier_alvo);
