...
#define VELOCIDADE_CAMERA 0.035
...
#define ALVOS_VELOCIDADE_INICIO 0.60
...
int main(int argc, char* argv[])
{
   ...
   // ShowCursor(false);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        model = Matrix_Scale(0.1f,0.1f,0.05f)*Matrix_Translate(vetor_alvos[0].x,ALTURA_ALVOS,30.0f);
        glUniformMatrix4fv(g_model_uniform, 1 , GL_FALSE , glm::value_ptr(model));
        glUniform1i(g_object_id_uniform, ALVO);
        DrawVirtualObject("Cube");

        model = Matrix_Scale(0.1f,0.1f,0.05f)*Matrix_Translate(vetor_alvos[1].x,ALTURA_ALVOS,0.0f);
        glUniformMatrix4fv(g_model_uniform, 1 , GL_FALSE , glm::value_ptr(model));
        glUniform1i(g_object_id_uniform, ALVO);
        DrawVirtualObject("Cube");

        model = Matrix_Scale(0.1f,0.1f,0.05f)*Matrix_Translate(vetor_alvos[2].x,ALTURA_ALVOS,-30.0f);
        glUniformMatrix4fv(g_model_uniform, 1 , GL_FALSE , glm::value_ptr(model));
        glUniform1i(g_object_id_uniform, ALVO);
        DrawVirtualObject("Cube");

        model = Matrix_Scale(0.1f,0.1f,0.05f)*Matrix_Translate(vetor_alvos[3].x,ALTURA_ALVOS,-60.0f);
        glUniformMatrix4fv(g_model_uniform, 1 , GL_FALSE , glm::value_ptr(model));
        glUniform1i(g_object_id_uniform, ALVO);
        DrawVirtualObject("Cube");
