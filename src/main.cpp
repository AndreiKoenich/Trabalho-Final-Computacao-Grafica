...
// renderização.
float g_CameraTheta = 10.0f; // Ângulo no plano ZX em relação ao eixo Z
...
glm::vec4 camera_position_c  = glm::vec4(1.0f,2.0f,2.0f,1.0f); // Ponto "c", centro da câmera
glm::vec4 camera_view_vector = glm::vec4(-2.0f,1.0f,0.0f,0.0f); // Vetor "view", sentido para onde a câmera está virada
...
int main(int argc, char* argv[])
{
    ...
    //LoadTextureImage("../../data/glocknormal.png"); // TextureImage2
    LoadTextureImage("../../data/glocktexture.png"); // TextureImage3
   // LoadTextureImage("../../data/gloclussion.png"); // TextureImage4
    ...
    ObjModel revolver("../../data/glock.obj");
    ComputeNormals(&revolver);
    BuildTrianglesAndAddToVirtualScene(&revolver);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        model = Matrix_Scale(0.1f,0.1f,0.0f)*Matrix_Translate(0.0f,5.5f,0.0f);
        glUniformMatrix4fv(g_model_uniform, 1 , GL_FALSE , glm::value_ptr(model));
        glUniform1i(g_object_id_uniform, TARGET);
        DrawVirtualObject("Cube");

        model = Matrix_Translate(0.0f,5.5f,0.0f)*Matrix_Scale(-0.1f,0.1f,0.1f);
        glUniformMatrix4fv(g_model_uniform, 1 , GL_FALSE , glm::value_ptr(model));
        ...
        {
            camera_position_c.x += (-1*w.x*SPEED);
            camera_position_c.y += (-1*w.y*SPEED);
            camera_position_c.z += (-1*w.z*SPEED);
        }
        ...
        {
            camera_position_c.x += (-1*u.x*SPEED);
            camera_position_c.y += (-1*u.y*SPEED);
            camera_position_c.z += (-1*u.z*SPEED);
        }
    }
