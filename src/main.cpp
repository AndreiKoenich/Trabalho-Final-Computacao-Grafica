...
void movimenta_esferas(Esfera vetor_esferas[])
{
    for (int i = 0; i < QUANTIDADE_ALVOS; i++)
    {
        if (vetor_esferas[i].centro_x >= LIMITE_DIR_ESFERA)
            vetor_esferas[i].direcao = DIRECAO_ESQUERDA;

        else if (vetor_esferas[i].centro_x <= LIMITE_ESQ_ESFERA)
            vetor_esferas[i].direcao = DIRECAO_DIREITA;
...
int main(int argc, char* argv[])
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (iniciar_jogo && !fim_jogo)
        {
            ...
            movimenta_esferas(vetor_esferas);
        ...
        if (iniciar_jogo && !fim_jogo)
        {
            ...
            {
                camera_position_c.x += delta_t*(-1*w.x*VELOCIDADE_CAMERA);
                //camera_position_c.y += (-1*w.y*VELOCIDADE_CAMERA);
                camera_position_c.z += delta_t*(-1*w.z*VELOCIDADE_CAMERA);
            }

            if (tecla_S_pressionada == 1)
            {
                camera_position_c.x += delta_t*(w.x*VELOCIDADE_CAMERA);
                //camera_position_c.y += delta_t*(w.y*VELOCIDADE_CAMERA);
                camera_position_c.z += delta_t*(w.z*VELOCIDADE_CAMERA);
            }

            if (tecla_D_pressionada == 1)
            {
                camera_position_c.x += delta_t*(u.x*VELOCIDADE_CAMERA);
                //camera_position_c.y += delta_t*(u.y*VELOCIDADE_CAMERA);
                camera_position_c.z += delta_t*(u.z*VELOCIDADE_CAMERA);
            }

            if (tecla_A_pressionada == 1)
            {
                camera_position_c.x += delta_t*(-1*u.x*VELOCIDADE_CAMERA);
                //camera_position_c.y += delta_t*(-1*u.y*VELOCIDADE_CAMERA);
                camera_position_c.z += delta_t*(-1*u.z*VELOCIDADE_CAMERA);
            }
        }
    }
